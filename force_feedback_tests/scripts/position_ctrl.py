...
if __name__ == '__main__':
    ...
    rospy.init_node(name="position_ctrl", anonymous=True)
    ...
    cmd_msg.settings.position_gains.kp = [1]
    cmd_msg.settings.position_gains.ki = [0]
    cmd_msg.settings.position_gains.kd = [0]
    cmd_msg.settings.position_gains.i_clamp = [0]
    hebi_wrap.send_command_with_acknowledgement(cmd_msg)

    _hold_position = False
    _hold_joint_position = None
